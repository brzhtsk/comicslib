...
export default function UploadPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      data.append('isTranslation', String(form.isTranslation));
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <input
            ...
            required
            placeholder="Ім'я автора оригінального твору"
            className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gray-400"
          />
          <p className="text-xs text-gray-400 mt-1">
            За замовчуванням – ваш нікнейм. Змініть якщо публікуєте чужий твір.
          </p>
        </div>
        ...
        <div>
          <label className="flex items-center gap-3 cursor-pointer">
            <div
              onClick={() => setForm((f) => ({ ...f, isTranslation: !f.isTranslation }))}
              className={`relative w-9 h-5 rounded-full transition-colors ${form.isTranslation ? 'bg-gray-900' : 'bg-gray-300'}`}
            >
              <span className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${form.isTranslation ? 'translate-x-4' : ''}`} />
            </div>
            <span className="text-sm text-gray-700">Зазначити мене як перекладача</span>
          </label>
          {form.isTranslation && (
            <p className="text-xs text-gray-500 mt-1 ml-12">
              Ваш нікнейм буде відображатись як перекладач на сторінці коміксу.
            </p>
          )}
        </div>
          ...
          <div className="flex gap-2">
            {STATUSES.map((s) => (
              <button
                ...
                className={`text-sm px-3 py-1.5 rounded-full border transition-colors ${form.status === s.value ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
              >
                {s.label}
              </button>
            ))}
          ...
          <div className="flex gap-2 flex-wrap">
            {genres.map((g) => (
              <button
                ...
                className={`text-sm px-3 py-1.5 rounded-full border transition-colors ${form.genreIds.includes(g.id) ? 'bg-gray-900 text-white border-gray-900' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
              >
                {g.name}
              </button>
            ))}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
            ...
            {isEdit && <span className="text-gray-400 font-normal ml-1">(можна не змінювати)</span>}
          </label>
          <div className="flex gap-4 items-start">
            {coverPreview && (
              <img
                src={coverPreview}
                alt="Прев'ю"
                className="w-24 rounded-lg object-cover aspect-[2/3] border border-gray-200 shrink-0"
              />
            )}
            <div>
              <input
                ...
                <div key={ch.id} className="flex items-center justify-between px-4 py-3">
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-gray-400">
                      {ch.pagesCount ?? ch.pages?.length ?? 0} стор.
                    </span>
                    <button

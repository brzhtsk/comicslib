...
export async function updateCommentHandler(req, res, next) {
  try {
    ...
    const { text } = req.body;
    if (!text?.trim()) return res.status(400).json({ message: 'Текст коментаря обовʼязковий' });
    res.json(await commentService.updateComment(id, req.user.id, text.trim()));
  } catch (err) { next(err); }
}
...
export async function toggleCommentLikeHandler(req, res, next) {
  try {
    const id = parseInt(req.params.id);
    if (isNaN(id)) return res.status(400).json({ message: 'Невірний id' });
    res.json(await commentService.toggleCommentLike(req.user.id, id));
  } catch (err) { next(err); }
}